...
function generateDynamicReasons(winnerKey, perfMatrix, weights) {
  const labels = {
  ...
  const contributions = Object.keys(labels).map((c) => ({
    crit: c,
    impact: weights[c] * perfMatrix[winnerKey][c],
  }));
...
async function initResultPage() {
  ...
  const reasons = generateDynamicReasons(winner.key, perfMatrix, weights);
